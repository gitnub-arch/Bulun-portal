...
const Afisha = () => {
  ...
  const handleTabChange = (value: string, type: 'day' | 'time', index: number) => {
        ...
        {AFISHA_ITEM.map((afisha, index) => (
          <div className="mb-[120px]">
            ...
            <div
              ...
              style={{
                backgroundColor:
                  activeTabs[index].day === afisha.day ? "#1875F0" : "#F2F2F2",
              }}
            >
                ...
                <TabsList className="">
                  {AFISHA_LIST.map((list) => (
                    <TabsTrigger
                      key={list.index}
                      className={`
          ${list.day === activeTabs[index].day ? "!text-[#FFFFFF] bg-[#1875F0]" : ""}`}
                      value={list.day}
                      onClick={() => handleTabChange(list.day, 'day', index)}
                    >
                      <div
                        className={`${
                          list.day === activeTabs[index].day ? "bg-[#1875F0]" : ""
                ...
                <TabsList className="gap-3">
                  {AFISHA_TIME.map((item) => (
                    <TabsTrigger
                      ...
                      value={item.time}
                      onClick={() => handleTabChange(item.time, 'time', index)}
                    >
